 ...
 */
 ...
 int __free(struct pcb_t *caller, int vmaid, int rgid)
 {
   ...
   struct vm_rg_struct *newfreerg = (struct vm_rg_struct *)malloc(sizeof(struct vm_rg_struct));
   if (!newfreerg) return -1;
   newfreerg->rg_start = rgnode.rg_start;
   newfreerg->rg_end = rgnode.rg_end;
   newfreerg->vmaid = vmaid;
   newfreerg->rg_next = NULL;
   /*enlist the obsoleted memory region */
   enlist_vm_freerg_list(caller->mm, newfreerg);
   //optional
   rgnode.rg_start = 0;
   rgnode.rg_end = 0;
   return 0;
   ...
   if (!PAGING_PAGE_PRESENT(pte)){ 
     ...
     if(MEMPHY_get_freefp(caller->mram, &freefpn)==0){
       ...
       *fpn = freefpn;
       ...
       pte_set_fpn(&mm->pgd[vicpgn], vicfpn);
 ...
 int find_victim_page(struct mm_struct *mm, int *retpgn)
 {
   ...
   if (!pg) return -1;  // no victim
   *retpgn = pg->pgn;           // select head as victim
   mm->fifo_pgn = pg->pg_next;  // remove from queue
   free(pg);
   return 0;
 }
